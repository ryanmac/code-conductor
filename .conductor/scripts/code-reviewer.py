...
class CodeReviewer:
    ...
    def analyze_code_quality(self, diff: str) -> None:
        ...
        for match in matches:
            ...
            next_func = re.search(r"^\+?\s*(def|function|func|class)\s+", diff[func_start:], re.MULTILINE)
        ...
        function_without_docstring = r"^\+\s*(def|class)\s+(\w+).*:\s*\n(?!\+\s*[\"']{3})"
    ...
    def suggest_tests(self, files: List[str]) -> None:
        ...
        for file in files:
            ...
            elif file.endswith(('.js', '.ts')) and not file.includes('test') and not file.includes('spec'):
